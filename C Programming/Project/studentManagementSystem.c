...
rec add(FILE *fp)
{
	...
	do
	{
		...
		while (fread(&temp, sizeof(rec), 1, fp))
			if (strcmp(temp.regNum, s.regNum) == 0)
			{
				isDuplicate = 1;
...
// Partition function for QuickSort - O(n log n) instead of O(n²)
int partition(rec arr[], int low, int high, char c)
{
	rec pivot = arr[high];
	int i = low - 1, j, shouldSwap;
	...
	for (j = low; j < high; j++)
	{
		...
		case '4':
			if (arr[j].m.percent <= pivot.m.percent)
				shouldSwap = 1;
			break;
...
void quickSort(rec arr[], int low, int high, char c)
{
	if (low < high)
	{
...
// Sort wrapper using QuickSort
void sort(rec sfac[], int n, char c)
{
	if (n > 1)
		quickSort(sfac, 0, n - 1, c);
}
